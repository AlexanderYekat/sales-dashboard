...
export default function SalesReportDashboard() {
  ...
  // Добавим функцию подсчета общих итогов
  const calculateTotalStats = (salesData: SalesData) => {
    const allReceipts = Object.values(salesData.months).flatMap(monthData =>
      Object.values(monthData.cashiers).flatMap(cashier =>
        Object.values(cashier.days).flatMap(day => day.receipts)
      )
    );
    return calculateReceiptTotals(allReceipts);
  };

  return (
    <div className="min-h-screen bg-gray-50 p-8">
      {salesData ? (
        <div className="max-w-7xl mx-auto space-y-12"> {/* Добавляем отступ между блоками */}
          {/* Заголовок */}
          <div className="mb-8">
          ...
          {/* Общие итоги */}
          <div className="space-y-4 border-b border-gray-200 pb-12"> {/* Добавляем разделитель */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {renderMetricCard(
                "Общие продажи",
                calculateTotalStats(salesData).sales,
                <span className="w-6 h-6 text-green-600 font-bold">₽</span>,
                "text-green-600"
              )}
              {renderMetricCard(
                "Общее сторно",
                calculateTotalStats(salesData).storno,
                <XCircle className="w-6 h-6 text-red-600" />,
                "text-red-600"
              )}
              {renderMetricCard(
                "Общие возвраты",
                calculateTotalStats(salesData).returns,
                <RefreshCcw className="w-6 h-6 text-orange-600" />,
                "text-orange-600"
              )}
              {renderMetricCard(
                "Общие отмены",
                calculateTotalStats(salesData).cancelOperations,
          ...
          {/* Данные по месяцам */}
          <div className="space-y-12"> {/* Отступ между месяцами */}
